...
from wg21_wiki_mcp import __version__, models
        ...
        if not sig.parameters:
            ...
            for param_name, param in sig.parameters.items():
                ...
                if param.default is inspect.Parameter.empty:
                    ...
                    description = ""
                lines.append(
                    f"| `{param_name}` | `{type_str}` | {default_str} | {description} |"
                )
            lines.append("")
...
def _models_section() -> str:
    ...
    lines = [
        "## Response models",
        "",
        "Structured tool outputs are Pydantic models in "
        "[models.py](../src/wg21_wiki_mcp/models.py). Public models:",
        "",
    ]
...
def render_api_md() -> str:
    ...
    version = __version__
    header = f"""# MCP tool API reference

> **Generated from source.** Do not edit this file manually.
> Regenerate with: `python scripts/build_api_docs.py`

Stable MCP host contract for `wg21-wiki-mcp` **{version}**. Tool names, parameter
